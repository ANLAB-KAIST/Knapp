...
public:
    Barrier(int _nThreads, int _device_id, int _stat_interval) : nThreads(_nThreads), c1(0), /*c2(0),*/ acc_us(0), acc_us_sq(0), num_used(0), ts(0), stat_interval(_stat_interval), device_id(_device_id), entry_count(0) {}
...
public:
    Barrier(int _nThreads) { nThreads = _nThreads; c1 = 0; pthread_spin_init(&lock, PTHREAD_PROCESS_SHARED); }
    void here(intptr_t iThread)
    {
        if(iThread)
        {
